...
treeSize :: Tree a -> Int
treeSize Empty                    = 0
treeSize (Node _ branch1 branch2) = 1 + treeSize branch1 +  treeSize branch2
...
treeMax Empty = 0
treeMax (Node value branch1 branch2)
  | value >= max_branch1 && value >= max_branch2  = value
  | max_branch1 >= max_branch2                    = max_branch1
  | otherwise                                     = max_branch2
    where
      max_branch1 = treeMax branch1
      max_branch2 = treeMax branch2
...
allValues condition (Node value branch1 branch2)  = condition value && allValues condition branch1 && allValues condition branch2 
...
mapTree f (Node value branch1 branch2)  = Node (f value) (mapTree f branch1) (mapTree f branch2) 
...
cull val (Node node_value branch1 branch2) = if val == node_value then Empty else Node node_value (cull val branch1) (cull val branch2)
...
isOrdered (Node value branch_left branch_right) = 
  allValues ((>=) value) branch_left    &&
  allValues ((<=) value) branch_right   &&
  isOrdered branch_left                 &&
  isOrdered branch_right
...
walk steps tree = valAtRoot $ foldr takeStep tree (reverse steps)
  where
    takeStep :: Step -> Tree a -> Tree a
    takeStep _ Empty                        = Empty
    takeStep StepL (Node _ branchL branchR) = branchL
    takeStep StepR (Node _ branchL branchR) = branchR
...
set (StepL : rest) val (Node x branchL branchR) = Node x (set rest val branchL) branchR  
set (StepR : rest) val (Node x branchL branchR) = Node x branchL (set rest val branchR)  
set [] val (Node x branchL branchR) = Node val branchL branchR
