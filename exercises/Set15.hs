...
statements xs ys = [\ x y -> x ++ " is " ++ y, \ x y -> x ++ " is not " ++ y] <*> xs <*> ys
...
calculator "negate" x = (* (-1)) <$> readMaybe x  
calculator "double" x = (* 2)    <$> readMaybe x
calculator _ x = Nothing  
...
validateAddress streetName streetNumber postCode = 
  check (length streetName <= 20) "Invalid street name" (Address streetName streetNumber postCode) <*
  check (not (any isAlpha streetNumber)) "Invalid street number" (Address streetName streetNumber postCode) <*
  check (not (any isAlpha postCode) && length postCode == 5) "Invalid postcode" (Address streetName streetNumber postCode)
...
twoPersons :: Applicative f =>
  ...
  -> f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = twoPerson' <$> name1 <*> age1 <*> employed1 <*> name2 <*> age2 <*> employed2
  where
    twoPerson' :: String -> Int -> Bool ->  String -> Int -> Bool -> [Person]
    twoPerson' name1 age1 employed1 name2 age2 employed2 = [Person name1 age1 employed1, Person name2 age2 employed2]
...
boolOrInt text = check (text == "True" || text == "False") "Not a Bool" (Left $ text == "True") <|>
                 check (not $ any isAlpha text) "Not an Int" (Right $ read text)
...
normalizePhone string = check (length stringNoSpaces <= 10) "Too long" stringNoSpaces <* traverse checkCharacter stringNoSpaces 
  where
    stringNoSpaces :: String
    stringNoSpaces = filter (not.isSpace) string

    checkCharacter :: Char -> Validation Char
    checkCharacter char = check ( (not.isAlpha) char) ("Invalid character: " ++ [char]) char
...
parseExpression expression
  | length (words expression) /= 3 = invalid ("Invalid expression: " ++ expression) 
  | symbol == "+" = liftA2 Plus  (parseVariable variable1) (parseVariable variable2)
  | symbol == "-" = liftA2 Minus (parseVariable variable1) (parseVariable variable2)
  | otherwise     = invalid ("Unknown operator: " ++ symbol) <* parseVariable variable1 <* parseVariable variable2
  where 
    [variable1, symbol, variable2] = words expression

    parseVariable :: String -> Validation Arg
    parseVariable var
      | all isNumber var                       = pure (Number   $ read var)
      | length var == 1 && isLetter (head var) = pure (Variable $ head var)
      | otherwise = invalid ("Invalid number: " ++ var) *> invalid ("Invalid variable: " ++ var)
...
instance Applicative Priced where
  pure x = Priced 0 x
  liftA2 f (Priced p1 x) (Priced p2 y) = Priced (p1+p2) (f x y)
...
tryAll f []       = empty
tryAll f (x:rest) = f x <|> tryAll f rest
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap func (Both x) = Both $ fmap (fmap func) x
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  ...
  liftA2 func (Both x) (Both y) = Both $ liftA2 (liftA2 func) x y 