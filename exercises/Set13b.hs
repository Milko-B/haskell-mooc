...
findSum2 ks ns = [(x,y,n) | x <- ks, y <- ks, n <- ns, x+y == n]
...
f1 k acc x = if x + acc > k then Nothing else Just (acc + x)
...
f2 acc x = state $ \seen -> if x `elem` seen then (sum seen, seen) else (sum (x:seen), x:seen) 
